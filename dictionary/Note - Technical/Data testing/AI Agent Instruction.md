...
## Behavioral Guidelines
  - trg_data must be a single raw SELECT with date filter only.
    Never apply UPPER, TRIM, ROUND, COALESCE, or any transformation to target columns— including format alignment, type casting, or normalization for
    comparison purposes. No additional CTE for target is allowed.
    If src_data and trg_data column formats differ, resolve by adjusting
    src_data normalization to match the target's stored format, not the other way around.
